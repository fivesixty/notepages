...
function block_invalid(req, res, id, protocols) {
  if (protocols.indexOf(req.extn) < 0) {
    res.send(util.inspect(req));
    // res.send("Invalid protocol.", 403);
    return true;
  } else if (!/^[a-zA-Z0-9_-]*$/.test(id)) {
    res.send({status: "failure", message: "Invalid page name (" + id + ") (only alphanumeric_- allowed)."});
    return true;
  }
  return false;
}
...
function prePage(req, res, next) {
  if(req.params.id.match(/\.[^\/]+$/)) {
    req.extn = req.params.id.substring(req.params.id.lastIndexOf('.') + 1);
    req.params.id = req.params.id.substring(0, req.params.id.lastIndexOf('.'));
  } else {
    req.extn = "html";
  }
  next();
...
app.post('/:id', prePage, express.bodyParser(), function(req, res, next) {
  if (block_invalid(req, res, req.params.id, ["json"])) return;
...
app.get('/:id', prePage, function(req, res, next) {
  if (block_invalid(req, res, req.params.id, ["json", "html"])) return;
  ...
  PageModel.findOne({iden: req.params.id}, function (err, post) {
    ...
    if (!err && post) {
      if (req.extn === "json") {
        ...
        page.editing = "false";
        page.content = sd.makeHtml(post.text);
      }
    } else {
      page.editing = "true"
      page.content = ""
